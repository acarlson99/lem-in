...
/*   Updated: 2019/01/20 13:15:55 by acarlson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (list[i])
	{
		...
		else if (valid_arr[i] && lens[i] > (*path_len_sum - lens[i]) / (*num_paths - 1) + ants_left / (*num_paths - 1))
		{
			...
			*num_paths = *num_paths - 1;
			ft_dprintf(2, "Ooh fuck we killin index %zu of len %zu num_ants: %u paths left: %zu path_len_sum: %zu\n", i, lens[i], ants_left, *num_paths, *path_len_sum);
